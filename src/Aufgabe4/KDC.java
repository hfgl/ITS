                    ...
                    if (tgsAuth.decrypt(tgsTicket.getSessionKey())) {
                        if (timeFresh(tgsAuth.getCurrentTime())) {
                            if (tgsAuth.getClientName().equals(tgsTicket.getClientName())) {
                                ...
                                System.out.println("#KDC: Serverkey in KDC K(S): " + serverTicketKey);
                                //Ticket mit K(C,S)
                                System.out.println("#KDC: Sessionkey in KDC K(C,TGS): " + tgsTicket.getSessionKey());

                                System.out.println("Creating internal ticket for serverticket");
                                String client = tgsAuth.getClientName();
                                serverticket = new Ticket(client, serverName, currentTime, currentTime + tenHoursInMillis, serverClientSessionKey);
                                //.. verschlüsselt mit K(S)
                                serverticket.encrypt(serverTicketKey);

                                //Response mit Sessionkey K(C,S)
                                response = new TicketResponse(serverClientSessionKey, nonce, serverticket);
